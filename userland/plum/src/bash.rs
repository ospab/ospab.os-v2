...
fn exec_command(cmd: &str, state: &mut ExecState) {
    ...
    if cmd.starts_with("export ") {
        // Delegate to plum export builtin via preprocess
        super::preprocess(cmd);
        return;
    }

    // echo — use plum preprocess which dispatches to terminal
...
/// Parse and execute an if block starting at line `i`.
/// Handles:
///   if condition; then   (or next line: then)
///   elif condition; then
///   else
///   fi
fn exec_if(lines: &[&str], i: usize, state: &mut ExecState) -> usize {
    ...
    // Extract condition: "if CONDITION" or "if CONDITION; then"
    let cond_str = {
        let after_if = raw[2..].trim(); // strip "if"
    ...
    // Advance past the "then" line
    let mut j = i + 1;
    // Skip any trailing "then" on its own line
    while j < lines.len() {
        let l = lines[j].trim().to_lowercase();
        if l == "then" { j += 1; break; }
        if l.ends_with("; then") || l.ends_with(";then") { j += 1; break; }
        // If the if line itself ended with "then", don't skip extra
        if raw.to_lowercase().contains("; then") || raw.to_lowercase().ends_with(" then") {
            break;
        }
        break; // condition was all inline, next line is body
    }

    // Collect then-block, elif chains, else-block
    ...
    while j < lines.len() {
        ...
        if depth == 1 {
            ...
            if l.starts_with("elif ") {
                // Treat rest as nested if
                if in_else {
                    else_lines.push(j);
                } else {
                    // If condition was true, we skip; if false, we need to eval elif
                    else_lines.push(j);
                }
                in_else = true;
                j += 1;
...
/// Evaluate a bash condition expression.
/// Handles: test-style [ ... ], string comparisons, arithmetic
fn eval_condition(cond: &str, state: &mut ExecState) -> bool {
    ...
    let inner = if (cond.starts_with('[') && cond.ends_with(']')) ||
        ...
        let inner = cond.trim_start_matches('[').trim_end_matches(']').trim();
        inner
    } else {
        cond
    };
    ...
    // -n string  (non-empty string test)
    ...
    // Numeric: a -eq b, a -ne b, a -lt b, a -le b, a -gt b, a -ge b
    ...
    // Treat non-empty string as truthy (last resort)
...
fn exec_for(lines: &[&str], i: usize, state: &mut ExecState) -> usize {
    let raw = lines[i].trim();
    // Syntax: for VAR in ITEMS [; do]
    let rest = raw[4..].trim(); // strip "for "
    ...
    // Items: split on spaces (until "; do" or end of line)
    ...
    while j < lines.len() {
        ...
        // If raw line itself has "; do", body starts next line
        if raw.to_lowercase().contains("; do") { break; }
        break;
    }

    // Collect body (handle nested for/while)
    let body_start = j;
    let mut depth = 1usize;
    while j < lines.len() {
        let l = lines[j].trim().to_lowercase();
...
fn exec_while(lines: &[&str], i: usize, state: &mut ExecState) -> usize {
    let raw = lines[i].trim();
    let cond_str_raw = &raw[6..]; // strip "while "
...
/// Detects and stores a function definition. Returns Some(next_line) if consumed.
fn parse_function_def(lines: &[&str], i: usize, state: &mut ExecState) -> Option<usize> {
    let raw = lines[i].trim();

    // Detect "name() {" or "name() " or "function name {"
...
/// Expand $VAR, ${VAR}, $((expr)) in a string using the exec state.
...
fn eval_arith(expr: &str, state: &ExecState) -> i64 {
    ...
    // Try simple binary operators
    for op in &["+", "-", "*", "/", "%"] {
        // Find the operator (right-to-left for left-associativity on +-)
        if let Some(pos) = s.rfind(op) {
            if pos == 0 { continue; } // unary
